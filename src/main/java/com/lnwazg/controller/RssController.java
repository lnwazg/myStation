...
public class RssController extends ParentController
{
    ...
    void viewRssAll()
    {
        try
        {
            ...
            List<String> contentList = (List<String>)JvmMemCacheLite.get("rssListSqlite", 5, TimeUnit.MINUTES);
            if (contentList == null)
            {
                guokeRssService.refreshRssContentCacheFromSqliteDB();
                contentList = (List<String>)JvmMemCacheLite.get("rssListSqlite", 5, TimeUnit.MINUTES);
            }
            okJson(contentList);
        }
        catch (Exception e)
